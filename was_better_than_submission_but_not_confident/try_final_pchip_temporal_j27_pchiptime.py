"""
try_final_pchip_temporal_j27_pchiptime_pchiptime.py
================================
...
New experiment added:
    ...
    - Select blend weight a by CV over Jan 27 observed interior cells:
          final = a * temporal_pred + (1-a) * cross_pred
    - Edges are not touched.
    - Non-Jan27 interiors are not touched.

Run:
    python try_final_pchip_temporal_j27_pchiptime_pchiptime.py --data cv_validation_system/dataset.csv
...
CFG_OUT_PREFIX = "try_final_pchip_temporal_j27_pchiptime"
...
def predict_non_edge_with_temporal_j27(df, row_idx, target_col, opt_type,
    ...
    Non-edge prediction:
      ...
      - Jan27: blend temporal same-contract extrapolation with cross-section prediction
    """
    ...
    if temporal_weight <= 0:
        ...
        info["temporal_weight"] = temporal_weight
    ...
    if ok and np.isfinite(tpred):
        pred = temporal_weight * tpred + (1.0 - temporal_weight) * cross_pred
        info["prediction"] = safe_iv(pred)
        info["source"] = info["source"] + "_temporal_j27_blend"
        info["selected_model"] = f"j27_temporal_a{temporal_weight:.2f}_plus_cross"
...
def grid_search_temporal_j27_weight(df, option_cols, cols_by_type, strike_map, type_map,
    ...
    records = []
    if not candidates:
        print("WARNING: No Jan27 interior temporal CV candidates. Using temporal_weight=0.")
    ...
    if val.empty:
        print("WARNING: Temporal CV produced no valid records. Using temporal_weight=0.")
        return 0.0, val

    summary = val.groupby("temporal_weight", as_index=False)["sq_err"].mean()
    summary = summary.rename(columns={"sq_err": "j27_interior_cv_mse"})
    best_row = summary.sort_values("j27_interior_cv_mse").iloc[0]
    best_a = float(best_row["temporal_weight"])

    if out_path is not None:
        val.merge(summary, on="temporal_weight", how="left").to_csv(out_path, index=False)

    base_mse = float(summary.loc[summary["temporal_weight"] == 0.0, "j27_interior_cv_mse"].iloc[0])
    best_mse = float(best_row["j27_interior_cv_mse"])

    print("\n" + "─" * 70)
    print(f"Jan27 interior PCHIP-temporal weight CV ({n_reps} reps × {mask_frac*100:.0f}% mask)")
    print("─" * 70)
    print(summary.to_string(index=False))
    print("─" * 70)
    print(f"Best temporal weight a : {best_a:.2f}")
    print(f"Baseline a=0 MSE       : {base_mse:.9f}")
    print(f"Best MSE               : {best_mse:.9f}")
    if base_mse > 0:
...
def main():
    ...
    if args.temporal_weight is not None:
        ...
        print(f"Using manual Jan27 temporal weight: {temporal_weight:.2f}")
    elif args.skip_cv:
        temporal_weight = 0.0
        print("Skipping temporal CV; using Jan27 temporal weight: 0.00")
    else:
        print("Running Jan27 temporal-weight CV...")
        temporal_weight, _ = grid_search_temporal_j27_weight(
    ...
    print(f"    Temporal weight    : {temporal_weight:.2f}")
    print("\nDiagnostics:")
