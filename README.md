...
The final surface is not generated by fitting one global model to all timestamps. Instead, each timestamp is treated as its own cross-sectional option smile. This matters because the IV surface is not stationary across time. The expiry-day regime, especially Jan 27, has much steeper and noisier IV behavior than the earlier dates.

The key modeling idea is:

```text
same timestamp + same option type + moneyness structure
```

For each row, CE values are filled using CE values from the same row, and PE values are filled using PE values from the same row. This prevents the method from mixing call and put smiles, which can have different skew behavior.

The model first converts each strike into moneyness:
...
```text
K = option strike
S = underlying price at that timestamp
x = moneyness
```

The completed surface is therefore built from local IV curves of the form:

```text
IV as a function of moneyness
```

For every missing IV value, the algorithm asks:

```text
Where is this missing point located inside the current timestamp's CE or PE smile?
```

It then classifies the missing value into one of two cases:
...
An interior gap is a missing value that has observed values on both sides in strike/moneyness space. This is an interpolation problem.

An edge wing is a missing value at the leftmost or rightmost missing part of the smile. This is an extrapolation problem, because only one side has observed values.

This distinction is the main structural choice in the final solution. Interior gaps are handled with local weighted regression plus PCHIP interpolation. Edge gaps are handled with progressive extrapolation and an edge ensemble.

The late surface jump is the key visual result. Near expiry, IV rises sharply and the wings become much harder to extrapolate. This is why a single global quadratic, a single time-series smoother, or a direct machine-learning model was not enough. The final method stays local, cross-sectional, and geometry-aware.

## 4. Filling Logic

The final script fills the dataset in a deterministic cell-by-cell order. For each missing value, it first parses the option metadata, determines the option type, computes moneyness, decides whether the missing value is an interior value or an edge value, and then sends it to the correct prediction engine.
...
This is the final modeling structure used to generate `submission_final.csv`.
