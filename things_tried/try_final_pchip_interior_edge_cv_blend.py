...
Minor edge CV gate added:
    The default edge model is still the validated try_final.py degree+bandwidth
    LOO edge ensemble. Before final filling, the script optionally tests a tiny
    blend toward a pure linear edge ensemble on hidden observed edge cells:
        edge_final = (1 - a) * try_final_edge + a * linear_edge
    If CV does not improve, a=0 is selected and edges remain exactly unchanged.
"""
...
# Minor edge safety experiment. Grid includes 0.0, so CV can choose “no change”.
EDGE_LINEAR_BLEND_GRID = np.round(np.arange(0.0, 0.5001, 0.05), 2)
EDGE_BLEND_CV_MASK_FRAC = 0.08
EDGE_BLEND_CV_REPS = 5
EDGE_BLEND_CV_SEED = 42
EDGE_BLEND_MIN_REL_IMPROVEMENT = 0.001  # require at least 0.1% CV improvement
...
def parse_args():
    ...
    if any('jupyter' in a or 'kernel' in a or '.json' in a for a in sys.argv[1:]):
        return types.SimpleNamespace(
            ...
            skip_cv=False,
            edge_linear_weight=None,
            skip_edge_blend_cv=False,
        )
    ...
    parser.add_argument("--edge-linear-weight", type=float, default=None,
                        help="Manual edge blend weight a. If omitted, selected by edge CV.")
    parser.add_argument("--skip-edge-blend-cv", action="store_true",
                        help="Skip edge blend CV and use --edge-linear-weight or 0.0.")
...
def _edge_predict_fixed_degree(x_obs, y_obs, x_target, global_median_iv, degree=1):
    """
    Edge-only helper for the tiny CV-gated linear blend.
    It keeps the same bandwidth grid but fixes the polynomial degree.
    """
    x_obs = np.asarray(x_obs, float)
    y_obs = np.asarray(y_obs, float)
    mask = np.isfinite(x_obs) & np.isfinite(y_obs)
    ...
    if len(y_obs) == 0:
        return float(global_median_iv), EDGE_LOCAL_POLY_BW, np.inf
    if len(y_obs) <= 2:
        bw = float(BANDWIDTH_GRID[len(BANDWIDTH_GRID) // 2])
        pred = local_poly_wls_pred(x_obs, y_obs, x_target, bw, degree=min(degree, len(y_obs)-1))
        return safe_iv(pred) if np.isfinite(pred) else float(global_median_iv), bw, np.inf
    ...
    pred = local_poly_wls_pred(x_obs, y_obs, x_target, best_bw, degree=degree)
    if not np.isfinite(pred):
        pred = local_poly_wls_pred(x_obs, y_obs, x_target, EDGE_LOCAL_POLY_BW, degree=degree)
    if not np.isfinite(pred):
        pred = global_median_iv
    return safe_iv(pred), best_bw, best_mse
...
def predict_edge_ensemble(df, row_idx, target_col, opt_type,
    ...
    out = {**claude_info,
           ...
           "edge_linear_weight": 0.0,
           "edge_linear_prediction": np.nan}

    # Minor CV-gated improvement: blend the existing try_final edge estimate
    # toward a pure linear edge ensemble. If CV selects a=0, this does nothing.
    if edge_linear_weight > 0:
        row = df.loc[row_idx]
        spot = row["underlying_price"]
        if pd.notna(spot) and spot > 0:
            x_target = strike_map[target_col] / spot
            x_c, y_c, _, _ = collect_edge_training_points_claude(
                row, target_col, opt_type, cols_by_type, strike_map, already_filled)
            x_co, y_co, _, _ = collect_edge_training_points_corrected(
                row, target_col, opt_type, cols_by_type, strike_map, already_filled)
            x_q, y_q, _, _ = collect_edge_training_points_quadratic(
                row, target_col, opt_type, cols_by_type, strike_map, already_filled)

            lin_c, _, _ = _edge_predict_fixed_degree(x_c, y_c, x_target, global_median_iv, degree=1)
            lin_co, _, _ = _edge_predict_fixed_degree(x_co, y_co, x_target, global_median_iv, degree=1)
            lin_q, _, _ = _edge_predict_fixed_degree(x_q, y_q, x_target, global_median_iv, degree=1)

            lin_components = {
                "claude": safe_iv(lin_c),
                "corrected": safe_iv(lin_co),
                "quadratic": safe_iv(lin_q),
            }
            lin_pred = (EDGE_BLEND_CLAUDE * lin_components["claude"] +
                        EDGE_BLEND_CORRECTED * lin_components["corrected"] +
                        EDGE_BLEND_QUADRATIC * lin_components["quadratic"])

            if np.isfinite(lin_pred):
                a = float(edge_linear_weight)
                blended = (1.0 - a) * out["prediction"] + a * lin_pred
                out["prediction"] = safe_iv(blended)
                out["source"] = "edge_blended_deg_selected_plus_linear_cv_gate"
                out["selected_model"] = f"edge_deg_selected_linear_blend_a{a:.2f}"
                out["edge_linear_weight"] = a
                out["edge_linear_prediction"] = safe_iv(lin_pred)
                out["edge_linear_component_predictions"] = lin_components
...
def predict_cell(df, row_idx, target_col, opt_type, cols_by_type,
    ...
    if edge:
        info = predict_edge_ensemble(
            df, row_idx, target_col, opt_type,
            cols_by_type, strike_map, global_median_iv, already_filled,
            edge_linear_weight=edge_linear_weight)
    else:
...
# ─────────────────────────────────────────────────────────────────────
# Edge linear-blend CV gate
# ─────────────────────────────────────────────────────────────────────

def grid_search_edge_linear_blend(df, option_cols, cols_by_type, strike_map, type_map,
                                  global_median_iv, out_path=None,
                                  mask_frac=EDGE_BLEND_CV_MASK_FRAC,
                                  n_reps=EDGE_BLEND_CV_REPS,
                                  seed=EDGE_BLEND_CV_SEED):
    """
    Hide observed cells, keep only cells that become edge cases under the mask,
    and choose a tiny blend toward a fixed-linear edge ensemble.

    Since 0.0 is in EDGE_LINEAR_BLEND_GRID, CV can choose the original file exactly.
    A small relative-improvement threshold avoids selecting noisy positive weights.
    """
    rng = np.random.default_rng(seed)
    observed = [(r, c) for r in df.index for c in option_cols if pd.notna(df.at[r, c])]
    records = []
    ...
    for rep in range(n_reps):
        k = max(1, int(len(observed) * mask_frac))
        idx = rng.choice(len(observed), k, replace=False)
        mask_set = {observed[i] for i in idx}
        df_m = df.copy()
        truths = {}
        for r, c in mask_set:
        ...
        for r, c in mask_set:
            ot = type_map[c]
            edge, _, _, _, _ = is_edge_missing(df_m.loc[r], c, ot, cols_by_type, strike_map)
            if not edge:
                continue
            true = truths[(r, c)]
            for a in EDGE_LINEAR_BLEND_GRID:
                info = predict_cell(df_m, r, c, ot, cols_by_type, strike_map,
                                    global_median_iv, {}, edge_linear_weight=float(a))
                pred = info["prediction"]
                if not np.isfinite(pred):
                    pred = global_median_iv
                records.append({
                    ...
                    "option_type": ot,
                    "strike": strike_map[c],
                    "edge_linear_weight": float(a),
                    "sq_err": (safe_iv(pred) - true) ** 2,
                    "edge_reason": info.get("edge_reason", ""),
    ...
    if val.empty:
        print("WARNING: edge linear-blend CV produced no edge records; using a=0.00")
        return 0.0, val

    summary = val.groupby("edge_linear_weight", as_index=False)["sq_err"].mean()
    summary = summary.rename(columns={"sq_err": "edge_cv_mse"})
    base_mse = float(summary.loc[summary["edge_linear_weight"] == 0.0, "edge_cv_mse"].iloc[0])
    best_row = summary.sort_values("edge_cv_mse").iloc[0]
    raw_best_a = float(best_row["edge_linear_weight"])
    raw_best_mse = float(best_row["edge_cv_mse"])

    # Conservative gate: use positive a only if it clearly beats a=0 on CV.
    if raw_best_mse < base_mse * (1.0 - EDGE_BLEND_MIN_REL_IMPROVEMENT):
        best_a = raw_best_a
    else:
        best_a = 0.0

    if out_path is not None:
        val.merge(summary, on="edge_linear_weight", how="left").to_csv(out_path, index=False)

    print("\n" + "─" * 70)
    print(f"Edge linear-blend CV ({n_reps} reps × {mask_frac*100:.0f}% mask)")
    print("─" * 70)
    print(summary.to_string(index=False))
    print("─" * 70)
    print(f"Raw best a       : {raw_best_a:.2f}")
    print(f"Selected a       : {best_a:.2f}")
    print(f"Baseline a=0 MSE : {base_mse:.9f}")
    print(f"Best raw MSE     : {raw_best_mse:.9f}")
    if base_mse > 0:
        print(f"Raw improvement  : {(base_mse - raw_best_mse) / base_mse * 100:.2f}%")
    print("─" * 70 + "\n")
...
def main():
    ...
    edge_blend_cv_out = out_dir / f"edge_linear_blend_validation_{out_prefix}.csv"
    ...
    if getattr(args, "edge_linear_weight", None) is not None:
        edge_linear_weight = float(args.edge_linear_weight)
        print(f"Using manual edge linear blend weight: {edge_linear_weight:.2f}")
    elif getattr(args, "skip_edge_blend_cv", False):
        edge_linear_weight = 0.0
        print("Skipping edge linear-blend CV; using a=0.00")
    else:
        print("Running edge linear-blend CV gate...")
        edge_linear_weight, _ = grid_search_edge_linear_blend(
            df, option_cols, cols_by_type, strike_map, type_map,
            global_median_iv, out_path=edge_blend_cv_out
        )

    if not getattr(args, "skip_cv", False):
        print("Running final CV validation with selected edge blend...")
    ...
    diag = {"missing_initial": len(missing_cells), "filled": 0,
            "fallback_global_median": 0, "pchip_used": 0,
            "edge_linear_blend_weight": edge_linear_weight,
            "edge_linear_blend_used": 0, "missing_after": None}
    rows = []
    ...
    for row_idx, col in tqdm(missing_cells, desc="Filling IVs"):
        ...
        if info.get("pchip_used", False):
            diag["pchip_used"] += 1
        if info.get("edge_linear_weight", 0.0) > 0:
            diag["edge_linear_blend_used"] += 1
        ...
        rows.append({
            ...
            "edge_linear_weight": info.get("edge_linear_weight", 0.0),
            "edge_linear_prediction": info.get("edge_linear_prediction", np.nan),
    ...
    print(f"✅  Edge blend CV  → {edge_blend_cv_out}")
    ...
    print(f"    Edge linear blend a: {edge_linear_weight:.2f}")
    print(f"    Edge linear blend used: {diag['edge_linear_blend_used']}")
    print("\nDiagnostics:")
