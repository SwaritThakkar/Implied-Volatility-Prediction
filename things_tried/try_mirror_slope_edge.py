...
No edge ensemble.
Cross-option transfer only as a mirrored OTM slope prior; no value-level bias correction.
...
Outputs
-------
    filled_dataset_try_mirror_slope_edge.csv
    submission_try_mirror_slope_edge.csv
    diagnostics_try_mirror_slope_edge.csv
    cross_section_diagnostics_try_mirror_slope_edge.csv
"""
...
# Edge-only rule.
# First edge missing value uses 2 nearest known-side points; deeper edge blocks
# progressively use more known-side points and prior filled edge predictions.
EDGE_EXTRA_POINT_SIDES = set()

# Mirrored-slope prior for OTM wings only:
#   CE right edge: slope should tilt upward as strike/moneyness increases.
#   PE left edge: slope should tilt upward as strike/moneyness decreases.
# The target slope magnitude is derived from the opposite option type's local
# slope near the same strike at the same timestamp.
MIRROR_SLOPE_SIDES = {("CE", "right"), ("PE", "left")}
MIRROR_SLOPE_PRIOR_WEIGHT = 1e-6
MIRROR_SLOPE_CLIP_MULT = 2.5
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="IV imputer: try.py non-edge logic + progressive linear edge logic with mirrored OTM slope prior."
    )
    parser.add_argument("--data", type=str, default=str(DEFAULT_DATA_PATH), help="Input dataset CSV.")
    parser.add_argument("--out-prefix", type=str, default="try_mirror_slope_edge", help="Output filename prefix.")
...
def fit_linear_with_slope_prior(x, y, slope_prior=None, prior_weight=0.0):
    """
    Linear least squares with an optional slope prior.

    Fits y = a*x + b, but when slope_prior is available it solves:
        min_{a,b} sum_i (a*x_i + b - y_i)^2 + w * (a - slope_prior)^2

    This keeps the edge rule linear while letting the mirrored side contribute
    slope/tilt information.
    """
    x = np.asarray(x, dtype=float)
    y = np.asarray(y, dtype=float)
    mask = np.isfinite(x) & np.isfinite(y)
    ...
    if slope_prior is not None and np.isfinite(slope_prior) and prior_weight > 0:
        lhs = lhs.copy()
        rhs = rhs.copy()
        lhs[0, 0] += float(prior_weight)
        rhs[0] += float(prior_weight) * float(slope_prior)
        fit_kind = "linear_mirror_slope_prior"
    else:
        fit_kind = "linear"
    ...
    return np.array([float(coeff[0]), float(coeff[1])]), fit_kind
...
def estimate_local_slope_from_cols(row, target_x, cols, strike_map):
    """
    Estimate local dIV/dmoneyness slope from the two nearest available points
    in a supplied option side. Uses same timestamp only.
    """
    ...
    for col in cols:
        ...
        if np.isfinite(x) and np.isfinite(val):
            pts.append((abs(x - target_x), x, float(val), col))

    if len(pts) < 2:
        return np.nan, []

    pts = sorted(pts, key=lambda t: t[0])[:2]
    pts = sorted(pts, key=lambda t: t[1])
    x1, y1, c1 = pts[0][1], pts[0][2], pts[0][3]
    x2, y2, c2 = pts[1][1], pts[1][2], pts[1][3]

    if abs(x2 - x1) <= 1e-12:
        return np.nan, [c1, c2]

    return float((y2 - y1) / (x2 - x1)), [c1, c2]


def mirrored_otm_slope_prior(row, target_col, opt_type, side, cols_by_type, strike_map, x_obs, y_obs):
    """
    Build a slope prior for OTM edge extrapolation from the opposite option type.

    CE right edge should tilt up as x increases: prior slope is +abs(mirror slope).
    PE left edge should tilt up as x decreases: prior slope is -abs(mirror slope).

    The magnitude comes from the opposite side's local slope near the same strike.
    The prior is clipped relative to the same-side fitted slope scale so one noisy
    mirrored pair cannot dominate the regression.
    """
    if (opt_type, side) not in MIRROR_SLOPE_SIDES:
    ...
    opposite = "PE" if opt_type == "CE" else "CE"
    target_x = strike_map[target_col] / spot
    raw_mirror_slope, mirror_cols = estimate_local_slope_from_cols(
        row=row,
        target_x=target_x,
        cols=cols_by_type[opposite],
        strike_map=strike_map,
    )

    if not np.isfinite(raw_mirror_slope):
        return np.nan, mirror_cols, raw_mirror_slope

    own_coeff, own_kind = fit_linear(x_obs, y_obs)
    own_slope = float(own_coeff[0]) if own_coeff is not None and np.isfinite(own_coeff[0]) else raw_mirror_slope
    max_abs = max(abs(own_slope), abs(raw_mirror_slope), 1e-6) * MIRROR_SLOPE_CLIP_MULT

    if opt_type == "CE" and side == "right":
        prior = abs(raw_mirror_slope)
    elif opt_type == "PE" and side == "left":
        prior = -abs(raw_mirror_slope)
    else:
        prior = raw_mirror_slope

    prior = float(np.clip(prior, -max_abs, max_abs))
    return prior, mirror_cols, raw_mirror_slope
...
def predict_edge_linear(df, row_idx, target_col, opt_type, cols_by_type, strike_map, global_median_iv, already_filled):
    ...
    if pd.isna(spot) or spot <= 0 or len(y_obs) == 0:
        ...
        slope_prior, mirror_cols, raw_mirror_slope = mirrored_otm_slope_prior(
            row=row,
            target_col=target_col,
            opt_type=opt_type,
            side=edge_side,
        ...
        coeff, fit_kind = fit_linear_with_slope_prior(
            x_obs,
            y_obs,
            slope_prior=slope_prior,
            prior_weight=MIRROR_SLOPE_PRIOR_WEIGHT,
        )
    ...
    return {
        ...
        "mirror_slope_prior": slope_prior if "slope_prior" in locals() else np.nan,
        "raw_mirror_slope": raw_mirror_slope if "raw_mirror_slope" in locals() else np.nan,
        "mirror_cols": mirror_cols if "mirror_cols" in locals() else [],
        **edge_info,
    }
...
def main():
    ...
    for row_idx, col in tqdm(missing_cells, desc="Filling IVs: try.py non-edge + linear edge"):
        ...
        rows.append({
            ...
            "mirror_slope_prior": info.get("mirror_slope_prior", np.nan),
            "raw_mirror_slope": info.get("raw_mirror_slope", np.nan),
            "mirror_cols": "|".join(map(str, info.get("mirror_cols", []))),
            "mirror_slope_prior_weight": MIRROR_SLOPE_PRIOR_WEIGHT if info["edge"] else np.nan,
        })
