"""
try_final_pchip_bucket.py
==========================

Same edge logic as try_final_pchip_interior.py / try_final.py:
    - edge predictions use the validated degree+bandwidth LOO selector.
    - edges are NOT touched by PCHIP.
...
Instead of one fixed PCHIP weight for all interiors, this script validates and
selects PCHIP blend weights by regime bucket:
    1. CE interior pre-Jan27
    2. PE interior pre-Jan27
    3. CE interior Jan27
    4. PE interior Jan27

Final interior prediction:
    pred = (1 - bucket_weight) * base_try_final + bucket_weight * pchip_pred

Why bucket-level, not row-level:
    Row-level weight search overfits because each row has too few strikes.
    Bucket-level search gives enough validation samples while allowing CE/PE and
...
Outputs:
    ...
    submission_try_final_pchip_bucket.csv
    diagnostics_try_final_pchip_bucket.csv
    cross_section_diagnostics_try_final_pchip_bucket.csv
    pchip_bucket_weight_validation_try_final_pchip_bucket.csv
"""
...
MIN_PCHIP_POINTS = 4
PCHIP_WEIGHT_GRID = np.round(np.arange(0.0, 0.5001, 0.05), 2)
INTERIOR_CV_MASK_FRAC = 0.08
INTERIOR_CV_REPS = 5
INTERIOR_CV_SEED = 42
DEFAULT_BUCKET_WEIGHT = 0.25

CFG_DATA       = str(DEFAULT_DATA_PATH)
CFG_OUT_PREFIX = "try_final_pchip_bucket"
...
def parse_args():
    ...
    if any('jupyter' in a or 'kernel' in a or '.json' in a for a in sys.argv[1:]):
        return types.SimpleNamespace(
            ...
            skip_cv=False,
            default_weight=DEFAULT_BUCKET_WEIGHT,
        )
    parser = argparse.ArgumentParser()
    parser.add_argument("--data",       type=str, default=CFG_DATA)
    parser.add_argument("--out-prefix", type=str, default=CFG_OUT_PREFIX)
    parser.add_argument("--skip-cv", action="store_true", help="Skip bucket CV and use --default-weight for all buckets.")
    parser.add_argument("--default-weight", type=float, default=DEFAULT_BUCKET_WEIGHT)
...
def bucket_name(row, opt_type):
    dt = row.get("datetime_parsed", pd.NaT)
    is_jan27 = False
    if pd.notna(dt):
        is_jan27 = pd.Timestamp(dt).date() == pd.Timestamp("2026-01-27").date()
    regime = "jan27" if is_jan27 else "pre_jan27"
    return f"{opt_type}_{regime}"
...
def pchip_same_row_pred(x_obs, y_obs, x_target):
    """Interior-only PCHIP interpolation. No extrapolation."""
...
# ─────────────────────────────────────────────────────────────────────
# Non-edge prediction: try.py base + bucket PCHIP blend
# ─────────────────────────────────────────────────────────────────────
...
def predict_non_edge_local_poly(df, row_idx, target_col, opt_type,
    ...
    bname = bucket_name(row, opt_type)
    pchip_w = float(bucket_weights.get(bname, DEFAULT_BUCKET_WEIGHT))
    ...
    pc = pchip_same_row_pred(x_obs, y_obs, x_target)
    if np.isfinite(pc) and pchip_w > 0:
        pred = (1.0 - pchip_w) * base_pred + pchip_w * pc
        source = "same_row_non_edge_wls_pchip_bucket_blend"
        selected_model = f"local_quad_wls_plus_pchip_{bname}_w{pchip_w:.2f}"
        pchip_used = True
    else:
        pred = base_pred
    ...
    return {"prediction": safe_iv(pred),
            ...
            "pchip_prediction": pc if np.isfinite(pc) else np.nan,
            "pchip_weight": pchip_w,
            "pchip_bucket": bname}


# ─────────────────────────────────────────────────────────────────────
# Edge training-point collectors copied from try_final_pchip_interior.py
# ─────────────────────────────────────────────────────────────────────
...
def predict_cell(df, row_idx, target_col, opt_type, cols_by_type,
                 strike_map, global_median_iv, already_filled, bucket_weights):
    row = df.loc[row_idx]
    ...
    if edge:
        ...
        info = predict_non_edge_local_poly(
            df, row_idx, target_col, opt_type,
            cols_by_type, strike_map, global_median_iv, bucket_weights)
    info["edge"] = bool(edge)
...
# ─────────────────────────────────────────────────────────────────────
# Bucket PCHIP validation
# ─────────────────────────────────────────────────────────────────────

def grid_search_pchip_bucket_weights(df, option_cols, cols_by_type, strike_map, type_map,
                                     global_median_iv, out_path=None,
                                     default_weight=DEFAULT_BUCKET_WEIGHT,
                                     mask_frac=INTERIOR_CV_MASK_FRAC,
                                     n_reps=INTERIOR_CV_REPS,
                                     seed=INTERIOR_CV_SEED):
    """
    Evaluate interior masked observed cells, grouped by bucket.
    For each bucket choose the PCHIP weight with lowest validation MSE.
    Edges are ignored; edge code remains unchanged.
    """
    buckets = ["CE_pre_jan27", "PE_pre_jan27", "CE_jan27", "PE_jan27"]
    fallback_weights = {b: float(default_weight) for b in buckets}

    if PchipInterpolator is None:
        print("WARNING: scipy unavailable; using default weights for all buckets.")
        return fallback_weights, pd.DataFrame(), pd.DataFrame()
    ...
    for rep in range(n_reps):
        chosen = rng.choice(len(observed), int(len(observed) * mask_frac), replace=False)
        ...
        for r, c in mask_set:
            ...
            spot = row["underlying_price"]
            x_obs, y_obs, _ = collect_same_row_points(row, ot, cols_by_type, strike_map)
            if pd.isna(spot) or spot <= 0 or len(y_obs) == 0:
                continue

            x_target = strike_map[c] / spot
            bw, _ = select_bandwidth_by_loo(x_obs, y_obs, BANDWIDTH_GRID)
            base_pred = local_poly_wls_pred(x_obs, y_obs, x_target, bw, degree=LOCAL_POLY_DEGREE)
            if not np.isfinite(base_pred):
                base_pred = global_median_iv
            ...
            bname = bucket_name(row, ot)
            true = truths[(r, c)]

            for w in PCHIP_WEIGHT_GRID:
                pred = (1.0 - float(w)) * base_pred + float(w) * pc
                records.append({
                    "rep": rep,
                    "row_idx": r,
    ...
    if val.empty:
        print("WARNING: no PCHIP validation rows; using default weights.")
        return fallback_weights, pd.DataFrame(), val

    summary = val.groupby(["bucket", "pchip_weight"], as_index=False)["sq_err"].agg(
        bucket_cv_mse="mean", n="count"
    )

    chosen_rows = []
    weights = {}
    for b in buckets:
        sub = summary[summary["bucket"] == b].copy()
        if sub.empty:
            weights[b] = float(default_weight)
            chosen_rows.append({
                "bucket": b,
                "chosen_weight": float(default_weight),
                "chosen_mse": np.nan,
                "baseline_mse_w0": np.nan,
                "n": 0,
                "reason": "no_validation_records_default_used",
            })
            continue

        best = sub.sort_values(["bucket_cv_mse", "pchip_weight"]).iloc[0]
        baseline = sub[sub["pchip_weight"] == 0.0]
        baseline_mse = float(baseline["bucket_cv_mse"].iloc[0]) if not baseline.empty else np.nan
        weights[b] = float(best["pchip_weight"])
        chosen_rows.append({
            "bucket": b,
            "chosen_weight": float(best["pchip_weight"]),
            "chosen_mse": float(best["bucket_cv_mse"]),
            "baseline_mse_w0": baseline_mse,
            "n": int(best["n"]),
            "reason": "cv_selected",
        })
    ...
    if out_path is not None:
        merged = val.merge(summary, on=["bucket", "pchip_weight"], how="left")
        merged.to_csv(out_path, index=False)
        chosen_path = Path(str(out_path).replace(".csv", "_chosen.csv"))
        chosen_df.to_csv(chosen_path, index=False)

    print("\n" + "─" * 72)
    print(f"Interior PCHIP bucket CV ({n_reps} reps × {mask_frac*100:.0f}% mask)")
    print("─" * 72)
    print(chosen_df.to_string(index=False))
    print("─" * 72)
    print("Selected bucket weights:")
    for k, v in weights.items():
        print(f"  {k:13s}: {v:.2f}")
    print("─" * 72 + "\n")

    return weights, chosen_df, val


# ─────────────────────────────────────────────────────────────────────
# CV Validation for the final combined model
# ─────────────────────────────────────────────────────────────────────

def run_cv_validation(df, option_cols, cols_by_type, strike_map, type_map,
                      global_median_iv, bucket_weights,
                      mask_frac=0.08, seed=123, n_reps=3):
    rng = np.random.default_rng(seed)
    results = []
    observed = [(r, c) for r in df.index for c in option_cols if pd.notna(df.at[r, c])]
    ...
    for rep in range(n_reps):
        ...
        for r, c in mask_set:
            ot = type_map[c]
            info = predict_cell(df_m, r, c, ot, cols_by_type, strike_map, global_median_iv, {}, bucket_weights)
            ...
            results.append({
                ...
                "pchip_bucket": info.get("pchip_bucket", "edge"),
                "rep": rep,
            })

    df_r = pd.DataFrame(results)
    print(f"\n{'─'*60}")
    print(f"Final CV Validation ({n_reps} reps × {mask_frac*100:.0f}% mask, {len(df_r)} predictions)")
    print(f"{'─'*60}")
    ...
    if len(df_r[~df_r.is_edge]):
        print("Interior by bucket:")
        print(df_r[~df_r.is_edge].groupby("pchip_bucket")["sq_err"].agg(["mean", "count"]).to_string())
    print(f"{'─'*60}\n")
    return df_r
...
def main():
    ...
    bucket_val_out  = out_dir / f"pchip_bucket_weight_validation_{out_prefix}.csv"
    final_cv_out    = out_dir / f"final_cv_validation_{out_prefix}.csv"
    ...
    if args.skip_cv:
        bucket_weights = {
            "CE_pre_jan27": float(args.default_weight),
            "PE_pre_jan27": float(args.default_weight),
            "CE_jan27": float(args.default_weight),
            "PE_jan27": float(args.default_weight),
        }
        print(f"Skipping bucket CV; using default weight {args.default_weight:.2f} for all interior buckets.")
    else:
        print("Running interior PCHIP bucket grid-search...")
        bucket_weights, chosen_df, val_df = grid_search_pchip_bucket_weights(
            df, option_cols, cols_by_type, strike_map, type_map, global_median_iv,
            out_path=bucket_val_out,
            default_weight=float(args.default_weight),
            mask_frac=INTERIOR_CV_MASK_FRAC,
            n_reps=INTERIOR_CV_REPS,
            seed=INTERIOR_CV_SEED,
        )
        final_cv = run_cv_validation(
            df, option_cols, cols_by_type, strike_map, type_map,
            global_median_iv, bucket_weights,
            mask_frac=0.08, seed=123, n_reps=3,
        )
        final_cv.to_csv(final_cv_out, index=False)
    ...
    for row_idx, col in tqdm(missing_cells, desc="Filling IVs"):
        ...
        rows.append({
            ...
            "pchip_weight": info.get("pchip_weight", 0.0),
            "pchip_bucket": info.get("pchip_bucket", "edge"),
    ...
    print(f"✅  Bucket CV file → {bucket_val_out}")
    if not args.skip_cv:
        print(f"✅  Final CV file  → {final_cv_out}")
