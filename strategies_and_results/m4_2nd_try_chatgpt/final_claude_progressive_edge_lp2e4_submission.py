"""
Final IV Imputer: Local-Polynomial WLS + Claude Progressive Edge LP(2e-4)
=========================================================================

This file converts the exact Claude progressive-edge idea into a final
submission-generating imputer.
...
Edge missing values:
    Use Claude's progressive local polynomial edge logic with bw = 2e-4.
...
Important
---------
This intentionally follows the Claude benchmark logic exactly for progressive
edge training-list updates:
    train_x.append(prediction)
    train_y.append(prediction)

Do not "fix" that to train_x.append(x_target). The goal of this file is to
implement the exact tested logic the user requested.

Run
---
    python final_claude_progressive_edge_lp2e4_submission.py --data dataset.csv

Outputs
-------
    filled_dataset_final_claude_edge_lp2e4.csv
    submission_final_claude_edge_lp2e4.csv
    diagnostics_final_claude_edge_lp2e4.csv
    cross_section_diagnostics_final_claude_edge_lp2e4.csv
"""
...
# Claude's final edge benchmark winner.
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Final IV imputer using local-poly WLS and Claude progressive edge LP(2e-4)."
    )
    parser.add_argument("--data", type=str, default=str(DEFAULT_DATA_PATH), help="Input dataset CSV.")
    parser.add_argument("--out-prefix", type=str, default="final_claude_edge_lp2e4", help="Output filename prefix.")
...
def collect_edge_training_points_claude(row, target_col, opt_type, cols_by_type, strike_map, already_filled):
    ...
    for prev_col in previous_cols:
        ...
        prev_pred = already_filled[prev_col]
...
def predict_cell(df, row_idx, target_col, opt_type, cols_by_type, strike_map, global_median_iv, already_filled):
    ...
    if edge:
        info = predict_edge_claude_local_poly(
            df=df,
            row_idx=row_idx,
            target_col=target_col,
            opt_type=opt_type,
...
def main():
    ...
    for row_idx, col in tqdm(missing_cells, desc="Filling IVs with Claude edge LP(2e-4)"):
        ...
        # Make this prediction available to the next missing value in the same edge block.
