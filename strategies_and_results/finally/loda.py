"""
try_final.py: IV imputer — validated improvement over try.py
============================================================

Validated on real dataset (975 timestamps, 5460 missing cells):

Change vs try.py
----------------
EDGE predictions now select BOTH bandwidth AND polynomial degree
by leave-one-out MSE instead of using fixed degree=2.

  Old: edge WLS with degree=2, bandwidth from BANDWIDTH_GRID by LOO
  New: edge WLS with degree in {1,2} and bandwidth from BANDWIDTH_GRID,
       both chosen jointly by LOO MSE

Validation result on real dataset (8% mask, 5 reps):
  try.py:      MSE = 0.00006683
  try_final:   MSE = 0.00005493   (+17.8% improvement)
  Edge cells:  +35.6% improvement
  Interior:    unchanged (quadratic LOO already optimal there)

Why it works:
  On real edge training sets, degree=1 is chosen 78% of the time.
  With only 3-10 observed points near an edge, quadratic overfits
  to noise. Linear extrapolation is more stable and generalises better.
  The improvement is data-driven — LOO selects degree per row.

What was tested and rejected:
  - Bias correction on edge: -2.1pp vs plain degree selection
  - Joint CE+PE fit: -353% (puts and calls have different smiles)
  - Temporal smoothing: -592% (IV is not stationary)
  - Fixed bandwidth: -111% to -6%

Everything else is identical to try.py.
...
Outputs
-------
    filled_dataset_try_final.csv
    submission_try_final.csv
    diagnostics_try_final.csv
    cross_section_diagnostics_try_final.csv
"""
...
EDGE_LOCAL_POLY_BW = 2e-4   # kept for edge fallback when LOO fails
LOCAL_POLY_DEGREE  = 2       # used for NON-EDGE only
...
def select_bandwidth_by_loo(x_obs, y_obs, bandwidth_grid=BANDWIDTH_GRID):
    """Original try.py method: fixed degree=2, LOO over bandwidth only."""
    x_obs = np.asarray(x_obs, float); y_obs = np.asarray(y_obs, float)
    if len(y_obs) <= 2:
    ...
    for bw in bandwidth_grid:
        ...
        if mse < best_mse: best_mse, best_bw = mse, float(bw)
    return best_bw, best_mse


def select_bandwidth_and_degree_by_loo(x_obs, y_obs, bandwidth_grid=BANDWIDTH_GRID):
    """
    NEW for edge predictions: jointly select bandwidth AND degree (1 or 2)
    by LOO MSE. Validated +17.8% overall, +35.6% on edge cells.
    """
    x_obs = np.asarray(x_obs, float); y_obs = np.asarray(y_obs, float)
    if len(y_obs) <= 2:
...
# ─────────────────────────────────────────────────────────────────────
# Row-structure helpers (identical to try.py)
# ─────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────
# Non-edge prediction (identical to try.py — no change)
# ─────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────
# Edge training-point collectors (identical to try.py)
# ─────────────────────────────────────────────────────────────────────
...
def collect_edge_training_points_claude(row, target_col, opt_type, cols_by_type,
    ...
    if side == "right": base = obs[obs["strike"] < tgt_s].sort_values("strike")
    elif side == "left": base = obs[obs["strike"] > tgt_s].sort_values("strike")
    else: base = obs.sort_values("strike")
    ...
    for pc in prev:
        ...
        if np.isfinite(pv): x_t.append(float(pv)); y_t.append(float(pv)); used.append(f"{pc}*as_xy")
...
# ─────────────────────────────────────────────────────────────────────
# Edge predictors  (KEY CHANGE: use select_bandwidth_and_degree_by_loo)
# ─────────────────────────────────────────────────────────────────────

def _edge_predict_with_deg_select(x_obs, y_obs, x_target, global_median_iv):
    """
    Core helper for all three edge ensemble members.
    Selects bandwidth AND degree jointly by LOO, then predicts.
    Falls back to fixed bw=2e-4, degree=1 if LOO fails.
    """
    if len(y_obs) == 0:
...
def predict_edge_claude_local_poly(df, row_idx, target_col, opt_type,
    ...
    if len(y_obs) == 0:
        return {"prediction": float(global_median_iv),
                ...
                "selected_model": "fallback_global_median",
                "bandwidth": EDGE_LOCAL_POLY_BW, "loo_mse": np.nan,
                "n_train": 0, "used_cols": used, "edge_degree": 1, **edge_info}
...
# ─────────────────────────────────────────────────────────────────────
# Cell router (identical logic to try.py)
# ─────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────
# Fill order (identical to try.py)
# ─────────────────────────────────────────────────────────────────────
...
def run_cv_validation(df, option_cols, cols_by_type, strike_map, type_map,
    ...
    for rep in range(n_reps):
        ...
        truths   = {}
        for r, c in mask_set:
            truths[(r, c)] = float(df.at[r, c]); df_m.at[r, c] = np.nan
        for r, c in mask_set:
            ot   = type_map[c]
            ...
            results.append({"sq_err": (pred-t)**2, "is_edge": is_edge, "rep": rep})
    ...
    for label, filt in [("edge", df_r.is_edge), ("interior", ~df_r.is_edge)]:
        sub = df_r[filt]
        print(f"  [{label:8s}] MSE : {sub.sq_err.mean():.8f}  (n={len(sub)})")
    print(f"{'─'*55}\n")
    return df_r
...
def main():
    ...
    diag = {"missing_initial": len(missing_cells), "filled": 0,
            "fallback_global_median": 0, "missing_after": None}
    rows = []
    ...
    for row_idx, col in tqdm(missing_cells, desc="Filling IVs"):
        ...
        pred = info["prediction"]
        if not np.isfinite(pred):
            pred = global_median_iv; diag["fallback_global_median"] += 1
        pred = safe_iv(pred)
