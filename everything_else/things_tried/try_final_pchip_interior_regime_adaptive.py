...
# Interior improvement over fixed 0.25 PCHIP blend.
# Validated idea from EDA:
#   1) use a slightly wider regime-specific bandwidth grid for Jan27
#      and a narrower safer grid on non-Jan27 days.
#   2) choose the PCHIP blend weight per row/option side by LOO over
#      a small conservative grid.
# Edges are still untouched.
PCHIP_INTERIOR_WEIGHT = 0.25          # fallback only
PCHIP_ADAPTIVE_WEIGHT_GRID = np.array([0.0, 0.10, 0.20], dtype=float)
INTERIOR_BW_GRID_J27 = np.array([7e-5, 1e-4, 1.5e-4, 2e-4, 3e-4, 5e-4], dtype=float)
INTERIOR_BW_GRID_OTHER = np.array([7e-5, 1e-4, 1.5e-4, 2e-4], dtype=float)
MIN_PCHIP_POINTS = 4
...
def interior_bandwidth_grid_for_row(row):
    if "datetime_parsed" in row.index and is_jan27_ts(row["datetime_parsed"]):
        return INTERIOR_BW_GRID_J27
    return INTERIOR_BW_GRID_OTHER
...
def select_adaptive_pchip_weight_by_row_loo(x_obs, y_obs, bandwidth_grid):
    """
    Choose the interior PCHIP blend weight using only observed cells in this row.

    This does NOT use the missing target. It leaves each observed point out, predicts
    it using the remaining points, and picks the weight with smallest row-level LOO MSE.
    The grid is intentionally small to avoid noisy per-row overfitting.
    """
    x_obs = np.asarray(x_obs, float)
    y_obs = np.asarray(y_obs, float)
    mask = np.isfinite(x_obs) & np.isfinite(y_obs)
    ...
    # Stable row-level bandwidth; same idea as Claude's validated EDA.
    row_bw, _ = select_bandwidth_by_loo(x_obs, y_obs, bandwidth_grid)

    best_w = 0.0
    best_mse = np.inf
    for w in PCHIP_ADAPTIVE_WEIGHT_GRID:
        errs = []
        for i in range(len(y_obs)):
            x_tr = np.delete(x_obs, i)
            y_tr = np.delete(y_obs, i)
            p_wls = local_poly_wls_pred(x_tr, y_tr, x_obs[i], row_bw, degree=LOCAL_POLY_DEGREE)
            if not np.isfinite(p_wls):
                continue
            p_pc = pchip_same_row_pred(x_tr, y_tr, x_obs[i])
            if float(w) > 0.0 and np.isfinite(p_pc):
                pred = (1.0 - float(w)) * p_wls + float(w) * p_pc
            else:
                pred = p_wls
            if np.isfinite(pred):
                errs.append((pred - y_obs[i]) ** 2)
        if errs:
...
def predict_non_edge_local_poly(df, row_idx, target_col, opt_type,
    ...
    # Regime-specific bandwidth grid: Jan27 has different curvature/noise behaviour.
    bw_grid = interior_bandwidth_grid_for_row(row)
    best_bw, loo_mse = select_bandwidth_by_loo(x_obs, y_obs, bw_grid)
    ...
    # Adaptive PCHIP blend only for true interior interpolation.
    # Weight is chosen by row-level LOO on observed same-row points.
    pchip_weight, pchip_weight_loo_mse = select_adaptive_pchip_weight_by_row_loo(
        x_obs, y_obs, bw_grid
    )
    pchip_pred = pchip_same_row_pred(x_obs, y_obs, x_target)
    if np.isfinite(pchip_pred) and pchip_weight > 0.0:
        pred = (1.0 - pchip_weight) * base_pred + pchip_weight * pchip_pred
        source = "same_row_non_edge_regime_bw_adaptive_pchip"
        selected_model = f"regime_bw_local_quad_plus_pchip_w{pchip_weight:.2f}"
        pchip_used = True
    else:
        pred = base_pred
...
def main():
    ...
    print("Interior logic : regime-specific BW + adaptive PCHIP weight grid [0, 0.10, 0.20]")
    ...
    if "pchip_weight" in diag_df.columns:
        print(f"    PCHIP weights used : {diag_df.loc[~diag_df.edge, 'pchip_weight'].value_counts().to_dict()}")
    print("\nDiagnostics:")
