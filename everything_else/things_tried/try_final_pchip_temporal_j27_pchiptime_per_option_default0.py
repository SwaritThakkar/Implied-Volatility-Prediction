"""
try_final_pchip_temporal_j27_pchiptime_per_option.py
================================

Best-known baseline retained:
    - Edge logic is exactly the try_final.py degree+bandwidth LOO edge ensemble.
    - Interior logic is WLS + PCHIP blend:
          cross_pred = 0.75 * local quadratic WLS + 0.25 * PCHIP

New experiment added:
    - Only for Jan 27 interior/non-edge cells.
    - Build a same-contract temporal prediction from the previous two Jan27 timestamps:
          temporal_pred = PCHIP(time, IV)[t]
    - Select blend weight a separately per option/contract by CV over Jan 27 observed interior cells:
          final = a_contract * temporal_pred + (1-a_contract) * cross_pred
...
Outputs:
    ...
    submission_try_final_pchip_temporal_j27_pchiptime.csv
    diagnostics_try_final_pchip_temporal_j27_pchiptime.csv
    temporal_pchip_j27_weight_validation_try_final_pchip_temporal_j27_pchiptime.csv
"""
...
# New Jan27 temporal blend config.
TEMPORAL_A_GRID = np.round(np.arange(0.0, 0.5001, 0.05), 2)
TEMPORAL_J27_CV_MASK_FRAC = 0.12
TEMPORAL_J27_CV_REPS = 8
TEMPORAL_J27_CV_SEED = 42

# Safety: temporal extrapolation can explode on expiry day.
# We clip around cross-section prediction, not absolute IV.
TEMPORAL_CLIP_WIDTH = 0.03

CFG_DATA       = str(DEFAULT_DATA_PATH)
CFG_OUT_PREFIX = "try_final_pchip_temporal_j27_pchiptime_per_option"
...
def parse_args():
    ...
    parser.add_argument("--skip-cv", action="store_true", help="Skip temporal CV and use --temporal-weight or 0.")
    parser.add_argument("--temporal-weight", type=float, default=None, help="Manual Jan27 temporal weight a.")
...
# ─────────────────────────────────────────────────────────────────────
# Interior cross-section and temporal predictions
# ─────────────────────────────────────────────────────────────────────

def predict_non_edge_cross_section(df, row_idx, target_col, opt_type,
                                   cols_by_type, strike_map, global_median_iv):
    """
    Current best interior baseline:
        0.75 * local quadratic WLS + 0.25 * PCHIP
    """
    row  = df.loc[row_idx]
    spot = row["underlying_price"]
...
def temporal_same_contract_pred(df_like, row_idx, col, cross_pred, clip_width=TEMPORAL_CLIP_WIDTH):
    """
    Same-contract temporal extrapolation using PCHIP over the last two Jan27 timestamps.

    We use the previous two timestamps only if both are also Jan27 and observed.
    With two points, PCHIP equals a shape-preserving secant extrapolator, but the
    implementation is explicitly PCHIP-based as requested.

    Uses df_like, so during CV it naturally respects hidden values.
    """
    if PchipInterpolator is None:
        return np.nan, False, "pchip_unavailable"

    if row_idx < 2:
        return np.nan, False, "not_enough_history"

    # Require current row and both history rows to be Jan27.
    if not (
        is_jan27_ts(df_like.at[row_idx, "datetime_parsed"])
        and is_jan27_ts(df_like.at[row_idx - 1, "datetime_parsed"])
        and is_jan27_ts(df_like.at[row_idx - 2, "datetime_parsed"])
    ):
        return np.nan, False, "history_not_jan27"

    v1 = df_like.at[row_idx - 1, col]
    v2 = df_like.at[row_idx - 2, col]

    if pd.isna(v1) or pd.isna(v2):
        return np.nan, False, "missing_history"

    try:
        v1 = float(v1)
        v2 = float(v2)
    except Exception:
        return np.nan, False, "bad_history"

    if not (np.isfinite(v1) and np.isfinite(v2)):
        return np.nan, False, "bad_history"

    # Use actual minutes from the older history point as the time axis.
    t0 = pd.Timestamp(df_like.at[row_idx - 2, "datetime_parsed"])
    t1 = pd.Timestamp(df_like.at[row_idx - 1, "datetime_parsed"])
    tt = pd.Timestamp(df_like.at[row_idx, "datetime_parsed"])

    x_hist = np.array([
        0.0,
        (t1 - t0).total_seconds() / 60.0,
    ], dtype=float)
    y_hist = np.array([v2, v1], dtype=float)
    x_target = float((tt - t0).total_seconds() / 60.0)

    if not np.all(np.isfinite(x_hist)) or not np.isfinite(x_target):
        return np.nan, False, "bad_time_axis"
    if x_hist[1] <= x_hist[0]:
        return np.nan, False, "bad_time_axis"

    try:
        model = PchipInterpolator(x_hist, y_hist, extrapolate=True)
        raw_pred = float(model(x_target))
    except Exception:
        return np.nan, False, "pchip_failed"

    if not np.isfinite(raw_pred) or raw_pred <= 0:
        return np.nan, False, "bad_temporal_pred"

    if np.isfinite(cross_pred):
        clipped = float(np.clip(raw_pred, cross_pred - clip_width, cross_pred + clip_width))
    else:
        clipped = raw_pred

    return safe_iv(clipped), True, "temporal_pchip_extrapolation_2pt"


def resolve_temporal_weight(temporal_weight, target_col):
    """
    temporal_weight may be:
      - scalar float: one global weight
      - dict: per-contract weights, with optional "__default__" fallback
    """
    if isinstance(temporal_weight, dict):
        return float(temporal_weight.get(target_col, temporal_weight.get("__default__", 0.0)))
    try:
        return float(temporal_weight)
    except Exception:
        return 0.0


def predict_non_edge_with_temporal_j27(df, row_idx, target_col, opt_type,
                                       cols_by_type, strike_map, global_median_iv,
                                       temporal_weight=0.0):
    """
    Non-edge prediction:
      - non-Jan27: unchanged cross-section prediction
      - Jan27: blend temporal same-contract PCHIP extrapolation with cross-section prediction

    The temporal blend weight is now contract-specific when temporal_weight is a dict:
        final = a[target_col] * temporal_pred + (1-a[target_col]) * cross_pred
    """
    ...
    a = resolve_temporal_weight(temporal_weight, target_col)

    if a <= 0:
        info["temporal_used"] = False
        info["temporal_prediction"] = np.nan
        info["temporal_weight"] = a
        info["temporal_reason"] = "weight_zero"
        return info

    if not is_jan27_ts(df.loc[row_idx, "datetime_parsed"]):
        info["temporal_used"] = False
        info["temporal_prediction"] = np.nan
        info["temporal_weight"] = 0.0
        info["temporal_reason"] = "not_jan27"
        return info

    tpred, ok, reason = temporal_same_contract_pred(df, row_idx, target_col, cross_pred)
    if ok and np.isfinite(tpred):
        pred = a * tpred + (1.0 - a) * cross_pred
        info["prediction"] = safe_iv(pred)
        info["source"] = info["source"] + "_temporal_j27_per_option_blend"
        info["selected_model"] = f"j27_temporal_contract_a{a:.2f}_plus_cross"
        info["temporal_used"] = True
        info["temporal_prediction"] = tpred
        info["temporal_weight"] = a
        info["temporal_reason"] = reason
    else:
        info["temporal_used"] = False
        info["temporal_prediction"] = np.nan
        info["temporal_weight"] = 0.0
...
# ─────────────────────────────────────────────────────────────────────
# Edge collectors and predictors: copied from try_final.py
# ─────────────────────────────────────────────────────────────────────
...
def predict_cell(df, row_idx, target_col, opt_type, cols_by_type,
    ...
    if edge:
        ...
        info = predict_non_edge_with_temporal_j27(
            df, row_idx, target_col, opt_type,
            cols_by_type, strike_map, global_median_iv,
...
def grid_search_temporal_j27_weight(df, option_cols, cols_by_type, strike_map, type_map,
                                    global_median_iv, out_path=None,
                                    mask_frac=TEMPORAL_J27_CV_MASK_FRAC,
                                    n_reps=TEMPORAL_J27_CV_REPS,
                                    seed=TEMPORAL_J27_CV_SEED):
    """
    CV only on Jan27 observed interior cells with valid two-timestamp history.

    Difference from previous version:
        old: one global a for all 28 contracts
        new: choose a separately for each contract

    For each contract:
        final = a_contract * temporal_pred + (1-a_contract) * cross_pred

    If a contract has no valid CV records, it gets "__default__" weight.
    """
    rng = np.random.default_rng(seed)

    # Candidate cells: observed, Jan27, non-edge in original row, has two previous valid same-contract IVs.
    candidates = []
    for r in df.index:
        if not is_jan27_ts(df.loc[r, "datetime_parsed"]):
            continue
        if r < 2:
            continue
        row = df.loc[r]
        for c in option_cols:
            if pd.isna(df.at[r, c]):
                continue
            ot = type_map[c]
            edge, _, _, _, _ = is_edge_missing(row, c, ot, cols_by_type, strike_map)
            if edge:
                continue
            if pd.notna(df.at[r - 1, c]) and pd.notna(df.at[r - 2, c]):
                candidates.append((r, c))

    records = []
    if not candidates:
        print("WARNING: No Jan27 interior temporal CV candidates. Using temporal weights=0.")
        return {"__default__": 0.0}, pd.DataFrame(), pd.DataFrame()

    for rep in range(n_reps):
        k = max(1, int(len(candidates) * mask_frac))
        chosen_idx = rng.choice(len(candidates), k, replace=False)
        mask_set = [candidates[i] for i in chosen_idx]
        ...
        for r, c in mask_set:
            ...
            tpred, ok, reason = temporal_same_contract_pred(df_m, r, c, cross_pred)
            if not ok or not np.isfinite(tpred):
                continue

            true = truths[(r, c)]
            for a in TEMPORAL_A_GRID:
                pred = float(a) * tpred + (1.0 - float(a)) * cross_pred
                records.append({
                    "rep": rep,
                    "row_idx": r,
                    ...
                    "option_type": ot,
                    "strike": strike_map[c],
                    "temporal_weight": float(a),
                    "sq_err": (safe_iv(pred) - true) ** 2,
                    "true": true,
    ...
    if val.empty:
        print("WARNING: Temporal CV produced no valid records. Using temporal weights=0.")
        return {"__default__": 0.0}, val, pd.DataFrame()

    # Default fallback for contracts with too few/no CV records. Kept at 0.0 so unknown contracts use pure cross-section.
    global_summary = val.groupby("temporal_weight", as_index=False)["sq_err"].mean()
    global_summary = global_summary.rename(columns={"sq_err": "global_j27_interior_cv_mse"})
    global_best_row = global_summary.sort_values("global_j27_interior_cv_mse").iloc[0]
    global_best_a = float(global_best_row["temporal_weight"])

    # Per-contract selection.
    per_contract_rows = []
    weight_map = {"__default__": 0.0}
    for contract, g in val.groupby("contract"):
        summary = g.groupby("temporal_weight", as_index=False)["sq_err"].agg(["mean", "count"]).reset_index()
        summary = summary.rename(columns={"mean": "contract_j27_interior_cv_mse", "count": "n"})
        best = summary.sort_values(["contract_j27_interior_cv_mse", "temporal_weight"]).iloc[0]
        best_a = float(best["temporal_weight"])

        base_rows = summary[summary["temporal_weight"] == 0.0]
        base_mse = float(base_rows["contract_j27_interior_cv_mse"].iloc[0]) if len(base_rows) else np.nan
        best_mse = float(best["contract_j27_interior_cv_mse"])
        n_records = int(summary["n"].max()) if len(summary) else 0

        weight_map[contract] = best_a
        per_contract_rows.append({
            "contract": contract,
            "option_type": str(g["option_type"].iloc[0]),
            "strike": int(g["strike"].iloc[0]),
            "best_temporal_weight": best_a,
            "base_mse_a0": base_mse,
            "best_mse": best_mse,
            "improvement_pct": ((base_mse - best_mse) / base_mse * 100.0) if np.isfinite(base_mse) and base_mse > 0 else np.nan,
            "n_records_per_weight": n_records,
        })

    contract_summary = pd.DataFrame(per_contract_rows).sort_values(["option_type", "strike"]).reset_index(drop=True)

    if out_path is not None:
        detailed = val.merge(global_summary, on="temporal_weight", how="left")
        detailed = detailed.merge(
            contract_summary[["contract", "best_temporal_weight", "base_mse_a0", "best_mse", "improvement_pct"]],
            on="contract",
            how="left",
        )
        detailed.to_csv(out_path, index=False)
        contract_summary.to_csv(Path(str(out_path).replace(".csv", "_per_contract_summary.csv")), index=False)
        global_summary.to_csv(Path(str(out_path).replace(".csv", "_global_summary.csv")), index=False)

    global_base_mse = float(global_summary.loc[global_summary["temporal_weight"] == 0.0, "global_j27_interior_cv_mse"].iloc[0])
    global_best_mse = float(global_best_row["global_j27_interior_cv_mse"])

    print("\n" + "─" * 86)
    print(f"Jan27 interior PCHIP-temporal PER-CONTRACT weight CV ({n_reps} reps × {mask_frac*100:.0f}% mask)")
    print("─" * 86)
    print("Global fallback summary:")
    print(global_summary.to_string(index=False))
    print("─" * 86)
    print("Per-contract selected weights:")
    print(contract_summary.to_string(index=False))
    print("─" * 86)
    print(f"Best global a if forced : {global_best_a:.2f}")
    print(f"Default fallback a      : {weight_map.get('__default__', 0.0):.2f}")
    print(f"Global baseline a=0 MSE : {global_base_mse:.9f}")
    print(f"Global best MSE         : {global_best_mse:.9f}")
    if global_base_mse > 0:
        print(f"Global improvement      : {(global_base_mse - global_best_mse) / global_base_mse * 100:.2f}%")
    print("─" * 86 + "\n")

    return weight_map, val, contract_summary
...
def main():
    ...
    temporal_val_out = out_dir / f"temporal_pchip_j27_weight_validation_{out_prefix}.csv"
    ...
    if args.temporal_weight is not None:
        temporal_weight = {"__default__": float(args.temporal_weight)}
        print(f"Using manual fallback Jan27 temporal weight for all contracts: {temporal_weight['__default__']:.2f}")
    elif args.skip_cv:
        temporal_weight = {"__default__": 0.0}
        print("Skipping temporal CV; using Jan27 temporal weight: 0.00 for all contracts")
    else:
        print("Running Jan27 per-contract temporal-weight CV...")
        temporal_weight, _, temporal_contract_summary = grid_search_temporal_j27_weight(
            df, option_cols, cols_by_type, strike_map, type_map,
            global_median_iv, out_path=temporal_val_out
        )
    ...
    diag = {
        ...
        "pchip_used": 0,
        "temporal_used": 0,
        "temporal_weight_default": resolve_temporal_weight(temporal_weight, "__missing__"),
        "temporal_weight_mode": "per_contract" if isinstance(temporal_weight, dict) and len(temporal_weight) > 1 else "global",
        "missing_after": None,
    }
    rows = []
    ...
    for row_idx, col in tqdm(missing_cells, desc="Filling IVs"):
        ...
        if info.get("pchip_used", False):
            diag["pchip_used"] += 1
        if info.get("temporal_used", False):
            diag["temporal_used"] += 1
        ...
        rows.append({
            ...
            "temporal_used": info.get("temporal_used", False),
            "temporal_prediction": info.get("temporal_prediction", np.nan),
            "temporal_weight": info.get("temporal_weight", 0.0),
            "temporal_weight_lookup": resolve_temporal_weight(temporal_weight, col),
            "temporal_reason": info.get("temporal_reason", ""),
    ...
    print(f"✅  Temporal CV    → {temporal_val_out}")
    ...
    print(f"    Temporal used      : {diag['temporal_used']}")
    if isinstance(temporal_weight, dict):
        shown = {k: v for k, v in temporal_weight.items() if k != "__default__"}
        print(f"    Temporal weight default : {temporal_weight.get('__default__', 0.0):.2f}")
        print(f"    Per-contract weights    : {len(shown)} contracts")
    else:
        print(f"    Temporal weight         : {float(temporal_weight):.2f}")
    print("\nDiagnostics:")
