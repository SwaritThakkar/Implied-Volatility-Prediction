"""
try_final_pchip_rowsearch.py
============================

Same edge logic as try_final.py / try_final_pchip_interior.py:
...
Instead of a fixed global PCHIP_INTERIOR_WEIGHT = 0.25, this version chooses
an interior blend weight PER ROW and OPTION TYPE by internal LOO search:

    pred(w) = (1 - w) * WLS + w * PCHIP

For the current timestamp and CE/PE side, it hides each actually observed
interior point one at a time, predicts it with WLS and PCHIP using the remaining
points, and chooses the weight w with lowest row-local LOO MSE.

Edges still use the exact try_final edge ensemble.
"""
...
# Row-wise interior search over PCHIP weight.
# w=0 means pure WLS, w=0.25 means old 75/25 blend, w=1 means pure PCHIP.
PCHIP_WEIGHT_GRID = np.array([0.0, 0.10, 0.15, 0.20, 0.25, 0.30, 0.35, 0.40, 0.50], dtype=float)
DEFAULT_PCHIP_WEIGHT = 0.25
MIN_PCHIP_POINTS = 4
MIN_ROW_SEARCH_POINTS = 3
...
CFG_OUT_PREFIX = "try_final_pchip_rowsearch"

def parse_args():
    import types, sys
    if any('jupyter' in a or 'kernel' in a or '.json' in a for a in sys.argv[1:]):
        return types.SimpleNamespace(data=CFG_DATA, out_prefix=CFG_OUT_PREFIX, skip_cv=False)
...
def observed_interior_indices_for_row(row, opt_type, cols_by_type):
    """Indices among same-side columns that are observed and not at observed edge."""
    cols = cols_by_type[opt_type]
    obs = [i for i, c in enumerate(cols) if pd.notna(row[c])]
    if len(obs) < 3:
        return []
    first, last = min(obs), max(obs)
    return [i for i in obs if first < i < last]


# ─────────────────────────────────────────────────────────────────────
# Row-wise PCHIP weight search for interiors only
# ─────────────────────────────────────────────────────────────────────

def row_pchip_weight_by_loo(row, opt_type, cols_by_type, strike_map, cache_key=None, cache=None):
    """
    Choose blend weight w for one timestamp and option type using only observed
    interior cells in that same row/side.

    This is a local model-selection step:
        - hide each observed interior point
        - predict it using WLS and PCHIP from the remaining points
        - choose w minimizing mean squared error

    Returns (best_w, best_mse, n_tests).
    """
    ...
    cols = cols_by_type[opt_type]
    test_indices = observed_interior_indices_for_row(row, opt_type, cols_by_type)
    if len(test_indices) < MIN_ROW_SEARCH_POINTS:
        ans = (DEFAULT_PCHIP_WEIGHT, np.inf, len(test_indices))
        if cache is not None and cache_key is not None:
            cache[cache_key] = ans
        return ans
    ...
    for i in test_indices:
        ...
        # Build training set from all observed same-side points except the held-out point.
        train_cols = [c for c in cols if c != target_col and pd.notna(row[c])]
        if len(train_cols) < 3:
            continue

        x_train = np.array([strike_map[c] / spot for c in train_cols], float)
        y_train = np.array([row[c] for c in train_cols], float)
        x_target = strike_map[target_col] / spot

        bw, _ = select_bandwidth_by_loo(x_train, y_train, BANDWIDTH_GRID)
        wls = local_poly_wls_pred(x_train, y_train, x_target, bw, degree=LOCAL_POLY_DEGREE)
        pc = pchip_same_row_pred(x_train, y_train, x_target)

        if not np.isfinite(wls) or not np.isfinite(pc):
            continue

        records.append((wls, pc, true_val))

    if len(records) < MIN_ROW_SEARCH_POINTS:
        ans = (DEFAULT_PCHIP_WEIGHT, np.inf, len(records))
        if cache is not None and cache_key is not None:
            cache[cache_key] = ans
        return ans
    ...
    for w in PCHIP_WEIGHT_GRID:
        pred = (1.0 - float(w)) * wls_vals + float(w) * pc_vals
        mse = float(np.mean((pred - true_vals) ** 2))
        if mse < best_mse:
            best_mse = mse
            best_w = float(w)

    ans = (best_w, best_mse, len(records))
    if cache is not None and cache_key is not None:
        cache[cache_key] = ans
    return ans
...
def predict_non_edge_local_poly(df, row_idx, target_col, opt_type,
    ...
    if pd.isna(spot) or spot <= 0 or len(y_obs) == 0:
        return {"prediction": float(global_median_iv),
                ...
                "pchip_weight": 0.0,
                "pchip_weight_mse": np.nan,
                "pchip_weight_tests": 0}
    ...
    best_w, weight_mse, n_tests = row_pchip_weight_by_loo(
        row=row,
        opt_type=opt_type,
        cols_by_type=cols_by_type,
        strike_map=strike_map,
        cache_key=(int(row_idx), opt_type),
        cache=row_weight_cache,
    )

    if np.isfinite(pchip_pred) and best_w > 0:
        pred = (1.0 - best_w) * base_pred + best_w * pchip_pred
        source = "same_row_non_edge_local_poly_wls_pchip_rowsearch_blend"
        selected_model = f"local_quad_wls_plus_pchip_row_w{best_w:.2f}"
        pchip_used = True
    else:
        pred = base_pred
        source = "same_row_non_edge_local_poly_wls"
        pchip_used = False
        best_w = 0.0 if not np.isfinite(pchip_pred) else best_w
    ...
    return {"prediction": safe_iv(pred),
            ...
            "pchip_weight": best_w,
            "pchip_weight_mse": weight_mse,
            "pchip_weight_tests": n_tests}
...
def predict_edge_claude_local_poly(df, row_idx, target_col, opt_type,
    ...
    if pd.isna(spot) or spot <= 0:
        return {"prediction": float(global_median_iv),
                ...
                "edge_position_in_block": np.nan,
                "pchip_used": False, "pchip_weight": 0.0}
...
def predict_cell(df, row_idx, target_col, opt_type, cols_by_type,
    ...
    if edge:
        ...
        info = predict_non_edge_local_poly(
            df, row_idx, target_col, opt_type,
            cols_by_type, strike_map, global_median_iv,
            row_weight_cache=row_weight_cache)
    info["edge"] = bool(edge)
...
def run_cv_validation(df, option_cols, cols_by_type, strike_map, type_map,
    ...
    for rep in range(n_reps):
        ...
        for r, c in mask_set:
            ...
            results.append({
                ...
                "pchip_weight": float(info.get("pchip_weight", 0.0)),
                "pchip_weight_tests": int(info.get("pchip_weight_tests", 0)),
                "rep": rep,
            })
    ...
    if len(df_r[~df_r.is_edge]):
        weights = df_r.loc[~df_r.is_edge, "pchip_weight"].value_counts().sort_index()
        print("  Interior weight counts:")
        for w, cnt in weights.items():
            print(f"    w={float(w):.2f}: {int(cnt)}")
    print(f"{'─'*55}\n")
    return df_r
...
def main():
    ...
    print(f"Interior PCHIP weight grid: {PCHIP_WEIGHT_GRID.tolist()}")
    ...
    for row_idx, col in tqdm(missing_cells, desc="Filling IVs"):
        ...
        rows.append({
            ...
            "pchip_weight_mse": info.get("pchip_weight_mse", np.nan),
            "pchip_weight_tests": info.get("pchip_weight_tests", 0),
    ...
    if diag["pchip_used"]:
        print("    Interior PCHIP weights:")
        for w, cnt in diag_df.loc[~diag_df.edge, "pchip_weight"].value_counts().sort_index().items():
            print(f"      w={float(w):.2f}: {int(cnt)}")
    print("\nDiagnostics:")
