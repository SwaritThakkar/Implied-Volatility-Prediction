"""
try_final_pchip_temporal_j27.py
================================
...
New experiment added:
    ...
    - Build a same-contract temporal prediction from the previous two timestamps:
          temporal_pred = 2 * IV[t-1, contract] - IV[t-2, contract]
...
Run:
    python try_final_pchip_temporal_j27.py --data cv_validation_system/dataset.csv

Outputs:
    filled_dataset_try_final_pchip_temporal_j27.csv
    submission_try_final_pchip_temporal_j27.csv
    diagnostics_try_final_pchip_temporal_j27.csv
    temporal_j27_weight_validation_try_final_pchip_temporal_j27.csv
"""
...
CFG_OUT_PREFIX = "try_final_pchip_temporal_j27"
...
def temporal_same_contract_pred(df_like, row_idx, col, cross_pred, clip_width=TEMPORAL_CLIP_WIDTH):
    """
    Same-contract temporal extrapolation:
        pred_t = 2 * IV[t-1] - IV[t-2]
    ...
    return safe_iv(clipped), True, "temporal_linear_extrapolation_2pt"
...
def grid_search_temporal_j27_weight(df, option_cols, cols_by_type, strike_map, type_map,
    ...
    print("\n" + "─" * 70)
    print(f"Jan27 interior temporal weight CV ({n_reps} reps × {mask_frac*100:.0f}% mask)")
    print("─" * 70)
...
def main():
    ...
    temporal_val_out = out_dir / f"temporal_j27_weight_validation_{out_prefix}.csv"
