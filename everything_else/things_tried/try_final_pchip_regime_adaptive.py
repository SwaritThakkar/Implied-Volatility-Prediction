"""
try_final_pchip_regime_adaptive.py
==================================

Submission generator using your best edge logic unchanged, plus Claude's
validated interior improvement:

Edges:
    Exactly the try_final.py edge ensemble:
      - Claude-style progressive edge
      - corrected progressive edge
      - local progressive edge component
      - each edge component selects bandwidth AND degree {1,2} by LOO

Interior / non-edge:
    1. Use regime-specific bandwidth grid:
         Jan27:     [7e-5, 1e-4, 1.5e-4, 2e-4, 3e-4, 5e-4]
         non-Jan27: [7e-5, 1e-4, 1.5e-4, 2e-4]
    2. For each row and option type, select PCHIP weight by LOO over:
         w in {0.0, 0.1, 0.2}
    3. Final interior prediction:
         pred = (1-w) * local_quadratic_WLS + w * PCHIP

This implements the validated combination from Claude's test:
    baseline total MSE: 0.000024315
    both total MSE:     0.000022773
...
Outputs:
    ...
    submission_try_final_pchip_regime_adaptive.csv
    diagnostics_try_final_pchip_regime_adaptive.csv
    cross_section_diagnostics_try_final_pchip_regime_adaptive.csv
"""
...
# Regime-specific interior grids from Claude's validation.
BANDWIDTH_GRID_JAN27 = np.array([7e-5, 1e-4, 1.5e-4, 2e-4, 3e-4, 5e-4], dtype=float)
BANDWIDTH_GRID_OTHER = np.array([7e-5, 1e-4, 1.5e-4, 2e-4], dtype=float)

# Per-row adaptive PCHIP blend weights for interiors.
PCHIP_WEIGHT_GRID = np.array([0.0, 0.1, 0.2], dtype=float)
MIN_PCHIP_POINTS = 4

CFG_DATA = str(DEFAULT_DATA_PATH)
CFG_OUT_PREFIX = "try_final_pchip_regime_adaptive"
...
def select_pchip_weight_by_row_loo(x_obs, y_obs, bandwidth_grid):
    """
    Per-row interior selector from Claude's validation.

    Uses the observed same-row same-option points only, chooses w in {0,0.1,0.2}
    by leave-one-out MSE for:
        pred_i = (1-w) * local_quad_WLS_i + w * PCHIP_i
    where local_quad_WLS_i uses one row-level bandwidth selected on the full
    observed row with the regime-specific grid.
    """
    x_obs = np.asarray(x_obs, float)
    y_obs = np.asarray(y_obs, float)
    mask = np.isfinite(x_obs) & np.isfinite(y_obs)
    ...
    if len(y_obs) < 5 or PchipInterpolator is None:
        return 0.0, np.inf

    row_bw = select_bandwidth_by_loo(x_obs, y_obs, bandwidth_grid)
    if isinstance(row_bw, tuple):
        row_bw = row_bw[0]

    best_w, best_mse = 0.0, np.inf
    for w_try in PCHIP_WEIGHT_GRID:
        loo_errs = []
        for i in range(len(y_obs)):
            xtr = np.delete(x_obs, i)
            ytr = np.delete(y_obs, i)
            xt = x_obs[i]
            yt = y_obs[i]
            p_lp = local_poly_wls_pred(xtr, ytr, xt, row_bw, degree=LOCAL_POLY_DEGREE)
            if not np.isfinite(p_lp):
                continue
            p_ph = pchip_same_row_pred(xtr, ytr, xt)
            if float(w_try) > 0.0 and np.isfinite(p_ph):
                pred = (1.0 - float(w_try)) * p_lp + float(w_try) * p_ph
            else:
                pred = p_lp
            if np.isfinite(pred):
                loo_errs.append((pred - yt) ** 2)
        if loo_errs:
...
# ─────────────────────────────────────────────────────────────────────
# Interior cross-section prediction: regime BW + adaptive PCHIP weight
# ─────────────────────────────────────────────────────────────────────

def interior_bandwidth_grid_for_row(df, row_idx):
    return BANDWIDTH_GRID_JAN27 if is_jan27_ts(df.loc[row_idx, "datetime_parsed"]) else BANDWIDTH_GRID_OTHER


def predict_non_edge_cross_section(df, row_idx, target_col, opt_type,
                                   cols_by_type, strike_map, global_median_iv):
    """
    Interior / non-edge prediction with Claude's validated improvement:
      - regime-specific bandwidth grid
      - per-row adaptive PCHIP weight chosen by LOO over {0,0.1,0.2}
    """
    row = df.loc[row_idx]
    spot = row["underlying_price"]
    ...
    if pd.isna(spot) or spot <= 0 or len(y_obs) == 0:
        return {
            ...
            "pchip_weight": 0.0,
            "pchip_weight_loo_mse": np.nan,
            "regime": "jan27" if is_jan27_ts(row.get("datetime_parsed")) else "other",
        }

    grid = interior_bandwidth_grid_for_row(df, row_idx)
    regime = "jan27" if is_jan27_ts(row["datetime_parsed"]) else "other"
    ...
    if not np.isfinite(base_pred):
        ...
        selected_model = "local_quadratic_wls_regime_bw"

    pchip_pred = pchip_same_row_pred(x_obs, y_obs, x_target)
    pchip_weight, pchip_weight_loo_mse = select_pchip_weight_by_row_loo(x_obs, y_obs, grid)

    if pchip_weight > 0.0 and np.isfinite(pchip_pred):
        pred = (1.0 - pchip_weight) * base_pred + pchip_weight * pchip_pred
        source = "same_row_non_edge_regime_bw_adaptive_pchip"
        selected_model = f"{selected_model}_pchip_w{pchip_weight:.2f}_{regime}"
        pchip_used = True
    else:
        pred = base_pred
        source = "same_row_non_edge_regime_bw_local_poly_wls"
        pchip_used = False
    ...
    return {
        ...
        "pchip_weight": pchip_weight,
        "pchip_weight_loo_mse": pchip_weight_loo_mse,
        "regime": regime,
    }
...
def run_cv_validation(df, option_cols, cols_by_type, strike_map, type_map,
    ...
    for rep in range(n_reps):
        ...
        for r, c in mask_set:
            ...
            results.append({
                ...
                "pchip_weight": info.get("pchip_weight", np.nan),
                "regime": info.get("regime", "edge" if is_edge else ""),
                "rep": rep,
            })
    ...
    if "pchip_weight" in df_r.columns:
        print("  Interior weights:", df_r.loc[~df_r.is_edge, "pchip_weight"].value_counts(dropna=False).sort_index().to_dict())
    print("─" * 70 + "\n")
    return df_r
...
def main():
    ...
    print(f"Interior BW grids : other={BANDWIDTH_GRID_OTHER.tolist()} jan27={BANDWIDTH_GRID_JAN27.tolist()}")
    print(f"PCHIP weight grid : {PCHIP_WEIGHT_GRID.tolist()}")
    ...
    for row_idx, col in tqdm(missing_cells, desc="Filling IVs"):
        ...
        if info.get("pchip_used", False):
            diag["pchip_used"] += 1
        w_used = info.get("pchip_weight", np.nan)
        if np.isfinite(w_used):
            if abs(w_used - 0.0) < 1e-12:
                diag["pchip_weight_0"] += 1
            elif abs(w_used - 0.1) < 1e-12:
                diag["pchip_weight_01"] += 1
            elif abs(w_used - 0.2) < 1e-12:
                diag["pchip_weight_02"] += 1
        ...
        rows.append({
            ...
            "pchip_weight": info.get("pchip_weight", np.nan),
            "pchip_weight_loo_mse": info.get("pchip_weight_loo_mse", np.nan),
    ...
    print(f"    PCHIP weight counts       : 0={diag['pchip_weight_0']}, 0.1={diag['pchip_weight_01']}, 0.2={diag['pchip_weight_02']}")
    print("\nDiagnostics:")
