"""
Matplotlib Contract Time-Series Dashboard: IV vs Time with Missing Points

Run this file from the same folder as dataset.csv, or keep the default path:

    python iv_contract_time_series_dashboard.py
...
The dashboard shows:
    ...
    missing points: timestamps where that contract's IV is missing

Controls:
    1. Contract Index slider:
        scrolls through all parsed option contracts.
    2. Previous / Next buttons:
        move one contract at a time.
    3. CE / PE radio buttons:
        filter contracts by option type.

No IV prediction or filling is done.
Missing IV values are placed on a horizontal reference band near the bottom
because their true IV is unknown.
"""
...
def make_contract_time_dashboard(df: pd.DataFrame, meta: pd.DataFrame) -> None:
    """
    Create interactive IV vs time dashboard for one contract at a time.
    """
    ...
    def update_plot(_=None):
        ...
        obs_x = timestamp_nums[observed_mask.to_numpy()]
        obs_y = series[observed_mask].to_numpy(dtype=float)

        miss_x = timestamp_nums[missing_mask.to_numpy()]
        ...
        title.set_text(
            f"IV vs Time: {contract_label(row)} | "
        ...
        ax.set_xlim(timestamp_nums.min(), timestamp_nums.max())
